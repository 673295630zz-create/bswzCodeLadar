...
def test_analyze_all_variables_option():
    code = '''
    int local_a;
    void A(void) {
        local_a = 3;
    ...
    sf = SourceFile(path=Path('a.c'), content=code)
    funcs = parse_functions('a.c', preprocess_code(code))
    result = analyze_relations('.', [sf], funcs, None, analyze_all_variables=True)
    assert any(r.target == 'local_a' for r in result.variable_relations)
